...
/**
 * Every `nn-slug.ca.mdx` dinámica must (1) have an ES sibling, (2) declare
 ...
 */
const ROOT = join('src', 'content', 'dinamicas');
...
describe('dinamicas CA sibling parity', () => {
  it('there is at least one CA dinámica', () => {
  ...
  for (const { familia, file } of caFiles) {
    ...
    it(`${familia}/${file} has an ES sibling with the same orden and is published CA prose`, () => {
      ...
      const expectedSlug = `dinamicas/${familia}/${file.replace(/\.mdx$/, '')}`;
