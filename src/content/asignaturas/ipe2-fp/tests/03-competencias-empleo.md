---
asignatura: ipe2-fp
unidad_relacionada: 3
title: "Test · Unidad 3 — Competencias para el empleo: personales, sociales y emocionales"
duracion_estimada: "12-15 min"
lang: es
estado: publicado
preguntas:
  - enunciado: "¿Cuál es la diferencia entre hard skills y soft skills?"
    opciones:
      - "Las hard skills son las difíciles y las soft skills las fáciles."
      - "Las hard skills son conocimientos técnicos de tu profesión, certificables; las soft skills son capacidades personales, sociales y emocionales útiles en cualquier puesto."
      - "Las hard skills sirven en oficinas y las soft skills en talleres."
      - "Las soft skills se aprenden en la FP y las hard skills en la universidad."
    correcta: 1
    explicacion: "Las **hard skills** (competencias técnicas) son los conocimientos específicos de tu profesión, que se aprenden y se certifican (programar, llevar contabilidad, manejar una máquina). Las **soft skills** (transversales) son capacidades personales, sociales y emocionales que sirven en cualquier puesto y sector, no se certifican pero se entrenan."
  - enunciado: "La unidad resume con una frase por qué importan tanto las competencias transversales. ¿Cuál?"
    opciones:
      - "Las empresas contratan por las transversales y despiden por las técnicas."
      - "Las empresas contratan por las técnicas y despiden por las transversales."
      - "Las empresas solo valoran las competencias técnicas."
      - "Las competencias transversales solo importan en puestos directivos."
    correcta: 1
    explicacion: "Las empresas **contratan por las competencias técnicas y despiden por las transversales**: casi nadie pierde el empleo porque no sepa hacer su trabajo, sino porque no sabe colaborar, comunicarse, gestionar su frustración o llevar bien los conflictos. Las soft skills son las más decisivas a largo plazo."
  - enunciado: "Según el modelo de Tuckman, ¿qué caracteriza a la fase de conflicto (storming) de un equipo?"
    opciones:
      - "Es señal de que el equipo ha fracasado y debe disolverse."
      - "Es una fase normal y necesaria por la que pasa casi cualquier equipo antes de rendir bien."
      - "Solo ocurre en equipos mal formados."
      - "Es la última fase, cuando se cierra el proyecto."
    correcta: 1
    explicacion: "La fase de **conflicto (storming)** —tensiones por el reparto de tareas, los criterios y el liderazgo— es una fase **normal y necesaria**, no un fracaso. Saber que casi todos los equipos pasan por ahí antes de rendir bien evita dramatizarla. Las cinco fases son: formación, conflicto, normalización, desempeño y finalización."
  - enunciado: "Según el modelo de Belbin, ¿qué tipo de equipo rinde mejor?"
    opciones:
      - "El formado por las personas más brillantes (todos \"cerebros\")."
      - "El que combina roles complementarios (cerebro, coordinador, implementador, evaluador, cohesionador)."
      - "El más numeroso posible."
      - "El que tiene un único líder que decide todo."
    correcta: 1
    explicacion: "Belbin demostró que los equipos formados solo por \"cerebros\" rinden peor que los **equilibrados**. Un equipo eficaz combina **roles complementarios**: cerebro (ideas), coordinador (organiza), implementador (ejecuta), evaluador (analiza riesgos) y cohesionador (cuida el clima). El equilibrio es lo que rinde, no reunir a los más brillantes."
  - enunciado: "¿Qué es la comunicación asertiva?"
    opciones:
      - "Imponer tu opinión interrumpiendo y descalificando."
      - "Callar y ceder para evitar el conflicto."
      - "Expresar lo que piensas, sientes y necesitas con claridad y respeto, sin agredir ni someterte."
      - "Hablar lo menos posible en las reuniones."
    correcta: 2
    explicacion: "La **asertividad** es decir lo que piensas con claridad y respeto, sin agredir ni someterte. Se contrasta con el estilo **pasivo** (callas y cedes, te frustras) y el **agresivo** (impones e interrumpes, ganas la discusión y pierdes al equipo). El estilo asertivo logra que te respeten y mantengas la relación."
  - enunciado: "¿En qué consiste la escucha activa?"
    opciones:
      - "Esperar tu turno para responder mientras el otro habla."
      - "Prestar atención plena, demostrarlo y comprobar que has entendido parafraseando."
      - "Asentir sin escuchar realmente."
      - "Tomar notas literales de todo lo que dice el otro."
    correcta: 1
    explicacion: "La **escucha activa** consiste en prestar atención plena a quien habla, demostrarlo (con la mirada, asintiendo, sin interrumpir) y **comprobar que has entendido** parafraseando (\"entonces, lo que necesitas es...\"). La mayoría escucha para responder, no para entender; quien escucha de verdad gana confianza y evita malentendidos."
  - enunciado: "El modelo de inteligencia emocional de Goleman distingue cinco componentes. ¿Cuáles son los tres componentes personales (hacia uno mismo)?"
    opciones:
      - "Empatía, habilidades sociales y motivación."
      - "Autoconciencia, autorregulación y motivación."
      - "Liderazgo, comunicación y resiliencia."
      - "Autoconciencia, empatía y habilidades sociales."
    correcta: 1
    explicacion: "Los cinco componentes de Goleman son: tres **personales** —**autoconciencia** (reconocer las propias emociones), **autorregulación** (gestionar los impulsos) y **motivación** (mover la conducta hacia objetivos)— y dos **sociales**: empatía y habilidades sociales. La buena noticia es que la inteligencia emocional se entrena a cualquier edad."
  - enunciado: "Según la unidad, ¿qué afirma Goleman sobre la relación entre inteligencia emocional y desempeño profesional?"
    opciones:
      - "El cociente intelectual predice el desempeño mejor que la inteligencia emocional."
      - "La inteligencia emocional predice el desempeño profesional mejor que el cociente intelectual, sobre todo en puestos de relación y liderazgo."
      - "Ambas son irrelevantes para el éxito laboral."
      - "Solo importa la inteligencia emocional en trabajos creativos."
    correcta: 1
    explicacion: "La tesis de Goleman, demostrada en numerosos estudios, es que la **inteligencia emocional predice el desempeño profesional mejor que el cociente intelectual**, especialmente en puestos de relación y liderazgo. La investigación desmintió la creencia de que el éxito dependía sobre todo del CI."
  - enunciado: "En la matriz de Eisenhower, ¿qué cuadrante es \"el más rentable y el más descuidado\"?"
    opciones:
      - "Urgente e importante (hazlo ya)."
      - "Importante pero no urgente (planifícalo): formación, prevención, planificación, relaciones."
      - "Urgente pero no importante (delégalo)."
      - "Ni urgente ni importante (elimínalo)."
    correcta: 1
    explicacion: "El cuadrante **importante pero no urgente** (planifícalo) es el **más rentable y el más descuidado**: aquí está el crecimiento profesional (formación, prevención, planificación, relaciones). El error común es vivir atrapado en las urgencias (cuadrante 1) y las interrupciones (cuadrante 3) sin tiempo para este. Gestionar bien el tiempo es, en el fondo, proteger el cuadrante 2."
  - enunciado: "¿Cómo se demuestran las competencias transversales en una entrevista por competencias?"
    opciones:
      - "Afirmando que las tienes (\"soy muy buen comunicador\")."
      - "Con ejemplos concretos estructurados con el método STAR."
      - "Mostrando tu expediente académico."
      - "Listándolas en una diapositiva."
    correcta: 1
    explicacion: "Las competencias transversales solo se desarrollan **en acción** y se demuestran en la entrevista con el **método STAR**: cada vez que resuelvas un conflicto, lideres una tarea o entregues a plazo bajo presión, anótalo como un ejemplo concreto. Afirmar que las tienes no convence; demostrarlas con un hecho real, sí."
---
