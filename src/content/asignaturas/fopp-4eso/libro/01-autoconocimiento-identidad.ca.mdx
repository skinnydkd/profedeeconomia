...
<RecursoDestacado href="/fopp-4eso/recursos/test-vocacional-riasec/" kicker="Test" title="Test vocacional RIASEC" descripcion="Trenta afirmacions per a obtindre el teu codi Holland i les famílies d'estudis i professions que millor hi encaixen. Servix de material de partida per al DAFO." colorVar="--color-fopp" />

<VocesDesacuerdo
  tema="Servixen d'alguna cosa els test de personalitat?"
  postura_a={{
    etiqueta: 'Servixen com a punt de partida',
    tesis: "Un qüestionari ordena el que un intuïx i dona un vocabulari per a parlar-ne. No decidix res, però obri la conversa amb noms concrets en lloc d'amb «no sé què se'm dona bé».",
  }}
  postura_b={{
    etiqueta: 'Prometen més del que poden',
    tesis: 'Molts test classifiquen en tipus tancats que canvien si els repetixes unes setmanes després, i quasi cap prediu si series bo en una faena. Un resultat amb aspecte científic pesa massa en algú de 15 anys.',
  }}
>
Convé distingir. Els qüestionaris d'**interessos professionals** —el RIASEC que veuràs als recursos d'esta unitat és el més usat— tenen darrere dècades d'investigació i mesuren alguna cosa estable: quin tipus d'activitats t'atrauen. Els que repartixen la gent en **tipus de personalitat** amb quatre lletres han sigut criticats just pel contrari: poca estabilitat en repetir-los i poca capacitat de predir el rendiment. **La regla pràctica és la mateixa per a tots: un test és una hipòtesi sobre tu, no un veredicte**, i es comprova provant coses, no omplint un altre test.
</VocesDesacuerdo>
...
- tipo: curriculum
  ...
  saberes:
    - "Bloc A: identitat personal, valors i fortaleses."
    - "Bloc A: propòsit personal i autoconcepte."
  competencias:
    - "Desenvolupar l'autoconeixement: identitat, valors, fortaleses i propòsit."
- tipo: section
  title: La identitat i els rols
  subtitle: "Qui eres i quin paper fas: dues coses distintes que es confonen cada dia"
- tipo: terms
...
- tipo: section
  ...
  subtitle: "Allò que de veres t'importa, posat per escrit i ordenat"
- tipo: concept
...
- tipo: section
  ...
  subtitle: "Huit intel·ligències, no una: on estàs fort i on et toca treballar"
- tipo: concept
...
- tipo: section
  ...
  subtitle: "Quatre caselles per a mirar de front el que portes i el que et falta"
- tipo: diagram
  diagrama: DAFOGrid
...
- tipo: recap
  ...
  items:
    - "Sabries separar la teua identitat personal dels rols que assumixes a casa, a classe i amb els amics."
    - "Sabries nomenar els teus cinc valors principals i ordenar-los en una jerarquia que pugues defendre."
    - "Sabries situar el teu perfil en les huit intel·ligències de Gardner sense reduir-te a una sola."
    - "Sabries construir un DAFO personal amb exemples concrets en les quatre caselles."
```
