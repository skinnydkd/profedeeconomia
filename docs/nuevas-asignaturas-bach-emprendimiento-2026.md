...
- [x] **Pasada visual y de rigor (items 5-8)**: imágenes Wikimedia (licencia verificada, ~3/unidad) en las 17 unidades de libro; diagramas (componentes SVG existentes) en las 17 unidades + las 6 fases del cuaderno GPE; auditoría de datos de los `RealExample` (correcciones puntuales); generador de slides mejorado (limpia JSX, viñetas por sección, slide "Lo esencial"). PDFs/diapositivas regenerados. Build verde (510 págs), 258 tests OK.
- [ ] **Pendiente**: revisión de contenido de Pau → flip a publicado (contenido + campo `estado` de las asignaturas en asignaturas.ts) + merge del PR #57. Si Pau edita contenido en la revisión, regenerar los PDFs/diapositivas afectados.
