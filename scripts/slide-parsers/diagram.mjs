...
import { dataUri as defaultDataUri } from './imports.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const root = resolve(__dirname, '../..');

export function renderDiagram(node, {
  asignatura, unitSlug, positionalIndex,
  existsFn = defaultExists,
  dataUriFn = defaultDataUri,
}) {
  ...
  const url = dataUriFn(abs) || `file:///${abs.replace(/\\/g, '/')}`;
