...
for (const slug of asignaturas) {
  const url = `http://localhost:${PORT}/${slug}/programacion/imprimir/`;
  const outDist = resolve(distDownloads, `${slug}-programacion.pdf`);
  const outPublic = resolve(publicDownloads, `${slug}-programacion.pdf`);

  console.log(`\n— Generando programación para ${slug}`);
  console.log(`  URL    : ${url}`);
  ...
  console.log(`✓ ${slug}-programacion.pdf listo`);
}
