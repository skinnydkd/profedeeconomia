 ...
 * spawning pattern, but targets /<slug>/programacion/imprimir/ and writes
 * <slug>-programacion.pdf.
 *
 * Usage:
...
{
  const probeUrl = `http://localhost:${PORT}/${asignaturas[0]}/programacion/imprimir/`;
  try {
...
const hasCa = (slug) => {
  const dir = resolve(root, `src/content/asignaturas/${slug}/programacion`);
...
for (const slug of asignaturas) {
 for (const loc of LOCALES) {
  ...
  const url = `http://localhost:${PORT}/${loc.prefix}${slug}/programacion/imprimir/`;
  const outDist = resolve(distDownloads, `${slug}-programacion${loc.suffix}.pdf`);
  const outPublic = resolve(publicDownloads, `${slug}-programacion${loc.suffix}.pdf`);

  console.log(`\n— Generando programación para ${loc.prefix}${slug}`);
  console.log(`  URL    : ${url}`);
  ...
  console.log(`✓ ${slug}-programacion${loc.suffix}.pdf listo`);
 }
}
...
console.log('\nProgramación(es) generada(s) con éxito.');
