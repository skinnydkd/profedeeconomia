#!/usr/bin/env node
/**
 * Build the printable «Entrevista a emprendedores» template PDF from the static
 * build (same HTTP-server + pagedjs pattern as build-proyecto-transversal-pdf).
 * Targets /emprendimiento/entrevista-emprendedores/imprimir/ and writes
 * entrevista-emprendedores-plantilla.pdf.
 *
 * Usage:
 *   node scripts/build-entrevista-pdf.mjs            # copy to public/downloads/
 *   node scripts/build-entrevista-pdf.mjs --in-dist  # only write to dist/downloads/
 */
...
// The template page is locale-aware and has a Valencian edition under /ca.
const JOBS = [
  { path: 'emprendimiento/entrevista-emprendedores/imprimir', out: 'entrevista-emprendedores-plantilla.pdf' },
  { path: 'ca/emprendimiento/entrevista-emprendedores/imprimir', out: 'entrevista-emprendedores-plantilla.ca.pdf' },
];
const PORT = 4336;
...
for (const job of JOBS) {
  ...
  console.log(`\n— Generando plantilla de entrevista (${job.path})`);
...
server.close();
console.log(`\n✓ Plantilla de entrevista (es + ca) lista.`);
